        ...
        {stops.map((stop, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-4 rounded-lg border border-subtle bg-surface p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="tabular mt-0.5 text-sm text-tertiary">
                    ...
                    {formatCurrency(stop.nights * stop.city.avgDailyCost, stop.city.currency)}
                  </p>
                </div>

                <div className="flex shrink-0 items-center gap-0.5">
                  <MoveButton
                    label={`Move ${stop.city.name} earlier`}
                    disabled={index === 0}
                    onClick={() => {
                  ...
                  <MoveButton
                    label={`Move ${stop.city.name} later`}
                  ...
                  <button
                    type="button"
                    aria-label={`Remove ${stop.city.name} from the route`}
                    onClick={() => {
              ...
              >
                Plan {stop.nights} days in {stop.city.name} →
              </Link>
            </motion.li>
          );
        })}
  ...
  return (
    <div className="flex items-center gap-3 py-2 pl-3.5">
      ...
      <p className="tabular text-mono-xs tracking-[0.09em] text-tertiary uppercase">
        {leg.mode === 'air' ? 'Fly' : 'Rail'} · {leg.distanceKm} km ·{' '}
        {formatDuration(leg.travelMinutes)}
      </p>
    </div>
  );
}
